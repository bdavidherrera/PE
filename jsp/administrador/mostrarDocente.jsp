                ...
                <div class="row">
                    <div class="col-12.5">
                        <div id="tabla-header" class="mb-4">
                            <h2 class="title-with-underline">Mostrar Docentes</h2>
                            <p class="text-muted mt-3">Actualización y elimininación de Docentes</p>
                        </div>
                        ...
                        <div id="tabla-contenido" class="mt-4">
                            <div class="table-responsive">
                                <table class="table table-custom table-hover">
                                    <thead>
                                        <tr>
                                            ...
                                            <th>Contraseña</th>
                                            <th>Titulos</th>
                                            <th>Director= 4 evaluador = 5</th>
                                            <th class="text-center">Acciones</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <sql:query var="result" scope="request" dataSource="${anteproyecto}">
                                            SELECT * FROM profesoruts WHERE identificacionp > 0;
                                        </sql:query>
                                        ...
                                        <c:forEach var="fila" items="${result.rows}">
                                            <tr>
                                                <td><c:out value="${fila.identificacionp}" /></td>
                                                <td><c:out value="${fila.nombre}" /> <c:out value="${fila.apellidos}" /> </td>
                                                <td><c:out value="${fila.correo}" /></td>
                                                <td><c:out value="${fila.password}" /></td>
                                                <td><c:out value="${fila.titulos}" /></td>
                                                <td><c:out value="${fila.fk_id_rolp}" /></td>
                                                <td class="text-center">
                                                    <a href="updateDocente.jsp?id=<c:out value='${fila.identificacionp}' />" class="btn action-btn edit-btn">
                                                        <i class="fas fa-edit"></i> Editar
                                                    </a>
                                                    <a href="vorrarDocente.jsp?id=<c:out value='${fila.identificacionp}' />" class="btn action-btn delete-btn">
                                ...
                                <div class="text-center mt-4">
                                        <a href="insertarDocente.jsp" class="btn new-btn">
