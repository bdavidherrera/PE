...
<title>ActualizarCoordinador</title>
</head>
<body>
        ...
        <div class="col-lg-6 col-md-8">
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-body p-5">
                    <h3 class="text-center mb-4 text-primary">Actualizar Coordinador</h3>
                    
                    <c:if test="${param.modifica != null}">
                        <c:choose>
                            <c:when test="${param.noidentificacionc != param.noidentificacionc_original}">
                                <sql:query var="verificacion" dataSource="${anteproyecto}">
                                    SELECT 1 WHERE
                                    EXISTS (SELECT 1 FROM administrador WHERE identificacion = ? OR password = ?) OR
                                    EXISTS (SELECT 1 FROM coordinador WHERE noidentificacionc = ? AND noidentificacionc != ? OR password = ?) OR
                                    ...
                                    <sql:param value="${param.noidentificacionc}" />
                                    <sql:param value="${param.password}" />
                                    <sql:param value="${param.noidentificacionc}" />
                                    <sql:param value="${param.noidentificacionc_original}" />
                                    <sql:param value="${param.password}" />
                                    <sql:param value="${param.noidentificacionc}" />
                                    <sql:param value="${param.password}" />
                                    <sql:param value="${param.noidentificacionc}" />
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <sql:update var="result" dataSource="${anteproyecto}">
                                            UPDATE coordinador
                                            SET noidentificacionc = ?,
                                                nombre = ?,
                                                apellidos = ?, 
                                                password = ?
                                            WHERE noidentificacionc = ?
                                            <sql:param value="${param.noidentificacionc}" />
                                        ...
                                        <c:if test="${result >= 1}">
                                            ...
                                            <div class="button-group mt-3">
                                                <a href="mostrarCoordinador.jsp" class="btn btn-primary w-100">Regresar a la lista</a>
                                            </div>
                                        </c:if>
                                    </c:otherwise>
                                </c:choose>
                    ...
                    <c:if test="${mostrarError == true || param.modifica == null}">
                        <sql:query var="coordinador" dataSource="${anteproyecto}">
                            SELECT * FROM coordinador WHERE noidentificacionc = ?
                            <sql:param value="${param.id != null ? param.id : param.noidentificacionc_original}"/>
                        </sql:query>
                        
                        <form method="post" class="needs-validation">
                            <c:forEach var="item" items="${coordinador.rows}">
                                <input type="hidden" name="noidentificacionc_original" value="${item.noidentificacionc}"/>
                                
                                <div class="mb-3">
                                    <label for="noidentificacionc" class="form-label">Identificación</label>
                                    <input type="number" class="form-control" name="noidentificacionc" value="${param.noidentificacionc != null ? param.noidentificacionc : item.noidentificacionc}" readonly required>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    <label for="fk_id_rolc" class="form-label">Rol</label>
                                    <input type="number" class="form-control" name="fk_id_rolc" value="${item.fk_id_rolc}" readonly>
                                    <small class="form-text text-muted">El rol no puede ser modificado</small>
                                </div>
                                
                                <div class="button-group">
                                    <button type="submit" class="btn btn-primary w-100">Actualizar Coordinador</button>
                        ...
                        <div class="button-group mt-3">
                            <a href="mostrarCoordinador.jsp" class="btn btn-primary w-100">Regresar</a>
                        </div>
