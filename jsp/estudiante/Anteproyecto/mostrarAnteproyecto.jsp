...
<sql:query var="resultado"  dataSource="${anteproyecto}">
...
<c:if test="${not empty resultado.rows}">
    <c:set var="enlaces" value="${resultado.rows[0]}" />
    <c:set var="enlaceAnteproyecto" value="${enlaces.anteproyecto}" />
    <c:set var="enlaceRadicado" value="${enlaces.radicadoant}"  />
    <c:set var="enlaceRadicadoComite" value="${enlaces.radicaante}" />
</c:if>
...
<title>MostrarAnteproyectosEs-</title>
</head>
<body>
            ...
            <div class="collapse navbar-collapse" id="navbarNav">
                <ul class="navbar-nav ml-auto">
                    <li class="nav-item active">
                        <a class="nav-link" href="../AnteproyectoEs.jsp"><i class="fas fa-home mr-1"></i> Regresar al menu de Anteproyecto</a>
                    </li>
                    <li class="nav-item">
                ...
                <div class="row">
                    <div class="col-12">
                        <div id="tabla-header" class="mb-4">
                            ...
                            <p class="text-muted mt-3">Puedes Adjuntar los respectivos documentos y ver el historial de versiones con las observaciones</p>
                        </div>
                        ...
                        <div id="tabla-contenido" class="mt-4">
                            <div class="table-responsive">
                                    ...
                                    <tbody>
                                        ...
                                        <c:forEach var="fila" items="${result.rows}">
                                            <tr>
                                                ...
                                                <td><a href= "<c:out value="${fila.anteproyecto}" />" target="_blank">${enlaceAnteproyecto}</a></td>
                                                <td><c:out value="${fila.nombredir}" /> 
                                                <td><c:out value="${fila.calificaciondir}"/>
                                                <td><a href="<c:out value="${fila.radicadoant}" />" target = "_blank"> ${enlaceRadicado}</a></td>
                                                <td><c:out value="${fila.nombreeva}" /></td>
                                                <td><c:out value="${fila.calificacioneva}" /></td>
                                                <td><c:out value="${fila.estadofinalcor}" /></td>
                                                <td><a href="<c:out value="${fila.radicaante}" />" target = "_blank"> ${enlaceRadicadoComite}</a></td>

                                                <c:if test="${fila.radicaante == 'Sin adjuntar'}">
                                                    <td class="text-center">
                                                        <a href="actualizarEnlace.jsp?id=<c:out value='${fila.id}' /> " 
                                                        class="btn action-btn edit-btn">
                                                            <i class="fas fa-edit"></i> Ingresar enlace documentos
                                                        </a>
                                                    </td>
                                                </c:if>
                                ...
                                <div class="text-center mt-4">
                                        <a href="../AnteproyectoEs.jsp" class="btn new-btn">
